...
const env = Env.getAll([
   ...
   */
  Env.newSecret('MONGODB_MAIN_LINK'), // mongodb host(s) as secret because it contains login credentials
  /**
   ...
   */
  Env.newEnv('MONGODB_MAIN_NAME', { defaultValue: 'app' }), // db name
  Env.newEnv('MONGODB_MAIN_AUTH', { defaultValue: 'app' }), // db auth name
  Env.newEnv('MONGODB_MAIN_REPLICASET', { defaultValue: 'rs0' }), // replica set name
  /**
   * With newVar, you set vars with default value
   * This default value will be use everywhere, even when APP_ENV is set to "staging"
   * or "production"
   */
  Env.newVar('MONGODB_SUBITOAPP_NAME', { defaultValue: 'subitoapp' }), // subitoapp collection name
  Env.newVar('MONGODB_SUBITOAPP_CURSOR', { defaultValue: 'createdAt' }), // subitoapp default cursor name
  Env.newEnv('MONGODB_SUBITOAPP_CSTYPE', { defaultValue: 'Date' }), // subitoapp default cursor type
