...
function copyPackages(appPath, variantPath, corePath, projectPath) {
  ...
  const variantPackageFilePath = variantPath ? `${appPath}/package.json` : null;
  const variantPackageLockFilePath = variantPath ?`${appPath}/package-lock.json` : null;
  ...
  let mergePC = mergeDeep(corePackageFileContent, appPackageFileContent);
  mergePC = mergeDeep(mergePC, variantPackageFileContent);
  ...
  let mergePLC = mergeDeep(corePackageLockFileContent, appPackageLockFileContent);
  mergePLC = mergeDeep(mergePLC, variantPackageLockFileContent);
  const packageLockContent = JSON.stringify(mergePLC, null, 2);
